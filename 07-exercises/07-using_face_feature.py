...
def find_max_contour(contours):  # roideki seklin uc noktalarini indislerini bulan fonksiyon tanimlandi
    ...
    max_area = 0  # roideki nesneler icinde maximum alan degerini tutacak degisken tanimlandi (amac sadece yuzu bulmasi)

    for i in range(len(contours)):  # kontur sayisi kadar dongude donulmesi saglandi
        area_face = cv2.contourArea(contours[i])  # her karede her kontur degeri icin alanlar bulundu

        if max_area < area_face:  # max alanin indis degerini tutacak kosul tanimlandi
            max_area = area_face  # alani en buyuk olan contour degeri kosula bagli bulundu
            max_i = i  # bulunan en buyuk alanli nesne yani sadece yuzun uc nokta indisleri degiskene atandi

    try:  # eger roide yuz bulunmazsa hata vermemesi icin try except blogu tanimlandi
        c = contours[max_i]  # yeni c degeri yuzun alaninin bulundugu indis noktasindaki kontur degeri olarak tanimlandi
        # yani bir karede sadece yuzun kontur degeri alinmasi saglandi
    except:
...
while True:
    ...
    roi = frame[100:370, 200:400]  # ekranin belirli bolgesi secildi (sadece yuz ile ilgilenildigi icin yuz kadar alan)
    cv2.rectangle(frame, (200, 100), (400, 370), (0, 0, 255), 0)

    hsv = cv2.cvtColor(roi, cv2.COLOR_BGR2HSV)  # yuzun icinde bulundugu aralik hsv formatina donusturuldu
    ...
    if len(contours) > 0:
        ...
        # yeni c degeri karede her konturu degil sadece yuzun kontur degerlerini icerir

        extLeft = tuple(c[c[:, :, 0].argmin()][0])
        # yuzun kontor degerleri arasinda en solda bulunan tum kontur degerleri icerisinde en kucuk olanin indisi alindi
